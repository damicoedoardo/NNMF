...
class NMF(BaseMatrixFactorizationRecommender):
    ...
    def __init__(self, URM_train):
        super(NMF, self).__init__(URM_train)
    ...
    def fit(self, num_factors=100,
        ...
        assert l1_ratio >= 0 and l1_ratio <= 1, \
            "{}: l1_ratio must be between 0 and 1, provided value was {}"\
            .format(self.RECOMMENDER_NAME, l1_ratio)

        if solver not in self.SOLVER_VALUES:
           raise ValueError("Value for 'solver' not recognized. Acceptable values are {}, provided was '{}'"
                            .format(self.SOLVER_VALUES.keys(), solver))

        if init_type not in self.INIT_VALUES:
           raise ValueError("Value for 'init_type' not recognized. Acceptable values are {}, provided was '{}'"
                            .format(self.INIT_VALUES, init_type))

        if beta_loss not in self.BETA_LOSS_VALUES:
           raise ValueError("Value for 'beta_loss' not recognized. Acceptable values are {}, provided was '{}'"
                            .format(self.BETA_LOSS_VALUES, beta_loss))

        self._print("Computing NMF decomposition...")

        nmf_solver = _NMF(n_components=num_factors,
                          init=init_type,
