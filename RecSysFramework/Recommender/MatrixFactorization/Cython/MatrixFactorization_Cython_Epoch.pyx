...
cdef class MatrixFactorization_Cython_Epoch:
    ...
    cdef dict latent_factors_initialization
    ...
    cdef int algorithm_is_funk_svd, algorithm_is_asy_svd, algorithm_is_BPR, algorithm_is_BPR_NNMF, algorithm_is_FUNK_NNMF, algorithm_is_PROB_NNMF

    cdef int[:] profile_length,
    cdef int[:] URM_train_indices, URM_train_indptr
    cdef double[:] URM_train_data

    cdef int[:] URM_train_coo_row, URM_train_coo_col
    cdef float[:] URM_train_coo_data

    #structure to save the sampled users and item
    ...
    ALGORITHM_NAME_VALUES = ["FUNK_SVD", "ASY_SVD", "MF_BPR", "BPR_NNMF", "FUNK_NNMF", "PROB_NNMF", "PROBMF"]
    ...
    def __init__(self, URM_train, n_factors = 1, algorithm_name = None,
        ...
        # for change the initialization values the parameters have to be changed in this file manually
        assert latent_factors_initialization['type'] in ['RANDOM', 'NORMAL'],\
            "latent factor initialization dictionary not well defined! \n has to be a DICTIONARY with one of " \
        ...
        URM_coo = check_matrix(URM_train, 'coo')
        self.URM_train_coo_row = URM_coo.row
        self.URM_train_coo_col = URM_coo.col
        self.URM_train_coo_data = URM_coo.data
        ...
        # initialize the structures count updates items and users
        ...
        self.latent_factors_initialization = latent_factors_initialization
        ...
        # Necessary for constant negative sampling
        np.random.seed(seed=42)
        srand(<unsigned int> int(42))
    ...
    def _init_latent_factors(self):
        ...
        self.algorithm_is_BPR_NNMF = False
        self.algorithm_is_FUNK_NNMF = False
        ...
        elif self.algorithm_name == "BPR_NNMF":
            self.algorithm_is_BPR_NNMF = True
        elif self.algorithm_name == "FUNK_NNMF":
            self.algorithm_is_FUNK_NNMF = True
        elif self.algorithm_name == "PROB_NNMF":
            self.algorithm_is_PROB_NNMF = True


        print("latent factors will be initialized: {}".format(self.latent_factors_initialization['type']))

        # W and H cannot be initialized as zero, otherwise the gradient will always be zero!
        if self.latent_factors_initialization['type'] == 'RANDOM':

            # bring interval [0,1] to the range decided by the user -> [new_min_range, new_max_range]
            new_max_range = self.latent_factors_initialization['range'][1]
            new_min_range = self.latent_factors_initialization['range'][0]
            old_max_range = 1
            old_min_range = 0

            OldRange = old_max_range-old_min_range
            NewRange = (new_max_range- new_min_range)

            if self.random_seed != -1:
                np.random.seed(self.random_seed)
            self.USER_factors = (np.random.rand(n_user_factors, self.n_factors)* NewRange)+new_min_range
            if self.random_seed != -1:
                np.random.seed(self.random_seed+1)
            self.ITEM_factors = (np.random.rand(n_item_factors, self.n_factors)* NewRange)+new_min_range

        elif self.latent_factors_initialization['type'] == 'NORMAL':
            init_mean = self.latent_factors_initialization['mean']
            init_std_dev = self.latent_factors_initialization['std']
    ...
    def epochIteration_Cython(self, **kwargs):
        ...
        elif self.algorithm_is_BPR_NNMF:
            self.epochIteration_Cython_BPR_NNMF(kwargs['data_user_similarity'],
                                                kwargs['inidices_user_similarity'],
                                                kwargs['data_item_similarity'],
                                                kwargs['indices_item_similarity'],
                                                kwargs['item_k'],
                                                kwargs['user_k']
                                               )
        elif self.algorithm_is_FUNK_NNMF:
            self.epochIteration_Cython_FUNK_NNMF(kwargs['data_user_similarity'],
                                                kwargs['inidices_user_similarity'],
                                                kwargs['data_item_similarity'],
                                                kwargs['indices_item_similarity'],
                                                kwargs['item_k'],
        ...
        elif self.algorithm_is_PROB_NNMF:
            self.epochIteration_Cython_PROB_NNMF(kwargs['data_user_similarity'],
                                                kwargs['inidices_user_similarity'],
                                                kwargs['data_item_similarity'],
                                                kwargs['indices_item_similarity'],
                                                kwargs['item_k'],
    ...
    def epochIteration_Cython_BPR_SGD(self):
        ...
        # Get number of available interactions
        # NOTE: an epoch is defined as a number of triples equal to the number of users
        cdef long num_total_batch = int(self.n_users / self.batch_size) + 1
        ...
        # at any iteration we avoid updating some factors
        # Renew dropout mask
        if self.dropout_flag:
            for factor_index in range(self.n_factors):
                self.factors_dropout_mask[factor_index] = rand() > self.dropout_quota
        ...
        for num_current_batch in range(num_total_batch):
            ...
            for num_sample_in_batch in range(self.batch_size):
                ...
                #update the user and item sample count structure
                self.user_update_count[u]+=1
                self.item_update_count[i]+=1
                self.item_update_count[j]+=1
    ...
    def epochIteration_Cython_BPR_NNMF(self,
                                       double[:,:] data_user_similarity,
                                       int[:,:] indeces_user_similarity,
                                       double[:,:] data_item_similarity,
                                       int[:,:] indeces_item_similarity,
                                       int item_k,
                                       int user_k,
                                      ):
        ...
        #we need to change the dimension since the parameter k is not reachable during the init of the cython epoch obj
        self.mini_batch_sampled_items = np.zeros(self.batch_size*2*item_k, dtype=np.int)
        self.mini_batch_sampled_users = np.zeros(self.batch_size*user_k, dtype=np.int)

        cdef long num_total_batch = int(self.n_users / self.batch_size) + 1

        # struct defined at the start of the file contains the user and the positive and negative samples
        cdef BPR_sample sample

        # the user and the positive and negative item
        cdef long u, i, j

        # declaration and initialization of a_f and b_f used to update the gradients check formula on paper...
        cdef double[:] a_f = np.zeros(self.n_factors, dtype=np.float64)
        cdef double[:] b_f = np.zeros(self.n_factors, dtype=np.float64)

        cdef long factor_index, num_current_batch, num_sample_in_batch, processed_samples_last_print, print_block_size = 500

        cdef double x_uij, sigmoid_user, sigmoid_item, local_gradient_i, local_gradient_j, local_gradient_u, local_gradient

        cdef double H_i, H_j, W_u, cumulative_loss = 0.0

        cdef double s_ki, s_kj, s_uv, p_vf
        cdef double q_kf_j = 0.0
        cdef double q_kf_i = 0.0
        cdef double A_f_acc_j = 0.0
        cdef double A_f_acc_i = 0.0

        cdef long i_neighbor_idx = 0
        cdef long j_neighbor_idx = 0
        cdef long i_neighbor, j_neighbor, neighbor_index, user_neighbor_index, u_neighbor, curr_neighbor
        ...
        for num_current_batch in range(num_total_batch):
            ...
            for num_sample_in_batch in range(self.batch_size):
                ...
                # Uniform user sampling with replacement
                sample = self.sampleBPR_Cython()
                #sample = self.sampleBPR_Cython_Pop_Correction()
                ...
                for factor_index in range(self.n_factors):
                    ...
                    if self.factors_dropout_mask[factor_index]:
                        # if entered here the dropout doesn't apply to this specific factor

                        #iterate on the k nearest neighbors of item i
                        for neighbor_index in range(item_k):

                            #retrieve the similarity of i with the selected neighbors if it is -1 there are no more
                            #neighboors for the item
                            s_ki = data_item_similarity[i,neighbor_index]
                            if s_ki != -1:
                                #retrieve the neighbor of i
                                i_neighbor = indeces_item_similarity[i, neighbor_index]

                                # NOTE: with respect to our formulas the matrix Q is not transposed it is
                                # (N_item*N_factor)
                                # retrieve the cells Qkf of the matrices Q for item i
                                q_kf_i = self.ITEM_factors[i_neighbor, factor_index]
                                A_f_acc_i += q_kf_i*s_ki
                            else:
                                break

                        #iterate on the k nearest neighbors of item j
                        for neighbor_index in range(item_k):

                            #retrieve the similarity of j with the selected neighbors
                            s_kj = data_item_similarity[j, neighbor_index]
                            if s_kj != -1:
                                #retrieve the neighbor of j
                                j_neighbor = indeces_item_similarity[j, neighbor_index]

                                # NOTE: with respect to our formulas the matrix Q is not transposed it is
                                # (N_item*N_factor)
                                # retrieve the cells Qkf of the matrices Q for item j
                                q_kf_j = self.ITEM_factors[j_neighbor, factor_index]
                                A_f_acc_j += q_kf_j*s_kj
                            else:
                                break

                        a_f[factor_index] = A_f_acc_i-A_f_acc_j

                        #TODO: implement also the possibility to use a different number of neighbors for the
                        # user and for the item ! you have to use a different letter on the for cycle below

                        #iterate on the k nearest neighbors of user u
                        for user_neighbor_index in range(user_k):

                            #retrieve the similarity between u and u_neighbor
                            s_uv = data_user_similarity[u,user_neighbor_index]

                            #if s_uv == -1 the user has no more neighbors
                            if s_uv != -1:
                                #retrieve the neighbor of the user u
                                u_neighbor = indeces_user_similarity[u, user_neighbor_index]

                                #retrieve the cell p_vf of the matrix P
                                p_vf = self.USER_factors[u_neighbor,factor_index]

                                #accumulate it on b_f
                                b_f[factor_index] += s_uv*p_vf
                            else:
                                break

                        #compute x_uij accumulating a_f*b_f
                        x_uij += a_f[factor_index]*b_f[factor_index]

                # Use gradient of log(sigm(-x_uij))
                sigmoid_item = 1 / (1 + exp(x_uij))
                sigmoid_user = sigmoid_item

                # is used only for print!
                cumulative_loss += x_uij
                ...
                for factor_index in range(self.n_factors):
                    ...
                    if self.factors_dropout_mask[factor_index]:
                        ...
                        # USER GRADIENT u,nn_u
                        for neighbor_index in range(user_k):

                            # retrieve one neighbor of the user u if it is -1 there are no more neighbors for u
                            curr_neighbor = indeces_user_similarity[u, neighbor_index]

                            if curr_neighbor != -1:
                                # Copy original value to avoid messing up the updates
                                p_vf = self.USER_factors[curr_neighbor, factor_index]

                                # compute the gradient for neighbor v (curr_neighbor)
                                local_gradient = data_user_similarity[u, neighbor_index]*(sigmoid_user*a_f[factor_index]- self.user_reg*p_vf)

                                #accumulate the gradient
                                self.USER_factors_minibatch_accumulator[curr_neighbor, factor_index] += local_gradient

                                #add user sample to minibatch corresponding to the computed gradient
                                if factor_index == 0:
                                    self._add_user_sample_in_minibatch(curr_neighbor)

                                    #update the user sampled count structure
                                    self.user_update_count[curr_neighbor]+=1

                                    #print(local_gradient)
                            else:
                                break

                        # ITEMS GRADIENTS i,j,nn_i,nn_j

                        while i_neighbor_idx<item_k or j_neighbor_idx<item_k:

                            if i_neighbor_idx < item_k:
                                i_neighbor = indeces_item_similarity[i,i_neighbor_idx]
                                if i_neighbor != -1:
                                    q_kf_i = self.ITEM_factors[i_neighbor, factor_index]
                                else:
                                    i_neighbor_idx = item_k
                            else:
                                i_neighbor = self.n_items+1

                            if j_neighbor_idx < item_k:
                                j_neighbor = indeces_item_similarity[j,j_neighbor_idx]
                                if j_neighbor != -1:
                                    q_kf_j = self.ITEM_factors[j_neighbor, factor_index]
                                else:
                                    j_neighbor_idx = item_k
                            else:
                                j_neighbor = self.n_items+1

                            if (i_neighbor < j_neighbor or j_neighbor_idx == item_k) and i_neighbor != -1\
                                    and i_neighbor_idx != item_k:
                                # i_neighbor is a i exclusive neighbor
                                local_gradient = data_item_similarity[i, i_neighbor_idx]*(sigmoid_item*b_f[factor_index]
                                                                                          -self.positive_reg * q_kf_i)

                                self.ITEM_factors_minibatch_accumulator[i_neighbor, factor_index] += local_gradient
                                i_neighbor_idx += 1
                                # add sample to minibatch corresponding to the computed gradient
                                if factor_index == 0:
                                    self._add_item_sample_in_minibatch(i_neighbor)

                                    self.item_update_count[i_neighbor]+=1

                            elif (i_neighbor > j_neighbor or i_neighbor_idx == item_k) and j_neighbor != -1 \
                                    and j_neighbor_idx!= item_k:
                                # j_neighbor is a j exclusive neighbor
                                local_gradient = data_item_similarity[j, j_neighbor_idx]*(sigmoid_item*(-b_f[factor_index])
                                                                                          - self.negative_reg * q_kf_j)

                                self.ITEM_factors_minibatch_accumulator[j_neighbor, factor_index] += local_gradient

                                #print('adding value {} in position {},{}'.format(local_gradient,j_neighbor,factor_index))

                                j_neighbor_idx += 1
                                # add sample to minibatch corresponding to the computed gradient
                                if factor_index == 0:
                                    self._add_item_sample_in_minibatch(j_neighbor)

                                    #update the item sampled count structure
                                    self.item_update_count[j_neighbor]+=1


                            elif (i_neighbor == j_neighbor) and i_neighbor != -1 and i_neighbor_idx != item_k:
                                # i_neighbor == j_neighbor is a shared neighbor -> q_kf_i == q_kf_j
                                local_gradient = (data_item_similarity[i, i_neighbor_idx] -
                                                  data_item_similarity[j, j_neighbor_idx]) * \
                                                 (sigmoid_item * b_f[factor_index] - self.item_reg * q_kf_i)
                    ...
                    # clean auxilliary structures here
                    b_f[factor_index] = 0.0
                    a_f[factor_index] = 0.0
    ...
    def epochIteration_Cython_FUNK_NNMF(self,
                                       double[:,:] data_user_similarity,
                                       int[:,:] indeces_user_similarity,
                                       double[:,:] data_item_similarity,
                                       int[:,:] indeces_item_similarity,
        ...
        #we need to change the dimension since the parameter k is not reachable during the init of the cython epoch obj
        self.mini_batch_sampled_items = np.zeros(self.batch_size*item_k, dtype=np.int)
        self.mini_batch_sampled_users = np.zeros(self.batch_size*user_k, dtype=np.int)
        ...
        # declaration and initialization of a_f and b_f used to update the gradients check formula on paper...
        cdef double[:] i_accumulator_f = np.zeros(self.n_factors, dtype=np.float64)
        cdef double[:] u_accumulator_f = np.zeros(self.n_factors, dtype=np.float64)

        cdef long factor_index, num_current_batch, num_sample_in_batch, processed_samples_last_print, print_block_size = 500

        cdef double r_ui, local_gradient

        cdef double H_i, H_j, W_u, cumulative_loss = 0.0

        cdef double s_ki, s_uv, rating
        cdef double q_kf = 0.0
        cdef double p_vf = 0.0

        cdef long i_neighbor_idx = 0
        cdef long i_neighbor, neighbor_index, user_neighbor_index, u_neighbor, curr_neighbor
        ...
        for num_current_batch in range(num_total_batch):
                    ...
                    if self.factors_dropout_mask[factor_index]:
                        ...
                        for neighbor_index in range(item_k):
                            ...
                            if s_ki != -1:
                                ...
                                # (N_item*N_factor)
                                # retrieve the cells Qkf of the matrices Q for item i
                                q_kf = self.ITEM_factors[i_neighbor, factor_index]
                                i_accumulator_f[factor_index] += q_kf*s_ki
                            else:
                                break
                        ...
                        for user_neighbor_index in range(user_k):
                            ...
                            if s_uv != -1:
                                ...
                                #accumulate it on b_f
                                u_accumulator_f[factor_index] += s_uv*p_vf
                            else:
                                break

                        #compute x_uij accumulating a_f*b_f
                        r_ui += i_accumulator_f[factor_index]*u_accumulator_f[factor_index]
                        #print('i_acc:{}\n u_acc:{}\n'.format(i_accumulator_f[factor_index], u_accumulator_f[factor_index]))

                #print(rating)
                #print(r_ui)
                #print('\n')
                #input()
                cumulative_loss += (rating-r_ui)**2
                ...
                for factor_index in range(self.n_factors):
                    ...
                    if self.factors_dropout_mask[factor_index]:
                        ...
                        for user_neighbor_index in range(user_k):
                            ...
                            # retrieve one neighbor of the user u if it is -1 there are no more neighbors for u
                            curr_neighbor = indeces_user_similarity[u, user_neighbor_index]
                            ...
                            if curr_neighbor != -1:
                                ...
                                # compute the gradient for neighbor v (curr_neighbor)
                                local_gradient = data_user_similarity[u, user_neighbor_index]*\
                                                 ((rating-r_ui)*i_accumulator_f[factor_index]-self.user_reg*p_vf)
                        ...
                        for neighbor_index in range(item_k):
                            ...
                            # retrieve one neighbor of the user u if it is -1 there are no more neighbors for u
                            curr_neighbor = indeces_item_similarity[i, neighbor_index]

                            if curr_neighbor != -1:
                                # Copy original value to avoid messing up the updates
                                q_kf = self.ITEM_factors[curr_neighbor, factor_index]

                                # compute the gradient for neighbor v (curr_neighbor)
                                local_gradient = data_item_similarity[i, neighbor_index]*\
                                                 ((rating-r_ui)*u_accumulator_f[factor_index]-self.item_reg*q_kf)

                                #print(local_gradient)
                                #input()

                                #accumulate the gradient
                                self.ITEM_factors_minibatch_accumulator[curr_neighbor, factor_index] += local_gradient

                                #add user sample to minibatch corresponding to the computed gradient
                                if factor_index == 0:
                                    self._add_item_sample_in_minibatch(curr_neighbor)

                                    #update the user sampled count structure
                                    self.item_update_count[curr_neighbor]+=1
                            else:
                                break

                    # clean auxilliary structures here
                    i_accumulator_f[factor_index] = 0.0
                    u_accumulator_f[factor_index] = 0.0
            ...
            if self.verbose and (processed_samples_last_print >= print_block_size or num_current_batch == num_total_batch-1):
                ...
                if current_time - last_print_time > 30 or num_current_batch == num_total_batch-1:
                    ...
                    print("{}: Processed {} ( {:.2f}% ) in {:.2f} seconds. FUNK loss {:.2E}. Sample per second: {:.0f}".format(
                        self.algorithm_name,
                        num_current_batch*self.batch_size,
                        100.0* num_current_batch/num_total_batch,
                        time.time() - last_print_time,
    ...
    def epochIteration_Cython_PROB_NNMF(self,
                                           double[:,:] data_user_similarity,
                                           int[:,:] indeces_user_similarity,
                                           double[:,:] data_item_similarity,
                                           int[:,:] indeces_item_similarity,
            ...
            for num_current_batch in range(num_total_batch):
                ...
                for num_sample_in_batch in range(self.batch_size):
                    ...
                    # apply the sigmoid function to rui
                    r_ui = 1/(1+exp(-r_ui))

                    # compute the derivative of the sigmoid function
                    sig_derivative = r_ui*(1-r_ui)
                        ...
                        if self.factors_dropout_mask[factor_index]:
                            ...
                            for user_neighbor_index in range(user_k):
                                ...
                                if curr_neighbor != -1:
                                    ...
                                    # compute the gradient for neighbor v (curr_neighbor)
                                    local_gradient = data_user_similarity[u, user_neighbor_index]*\
                                                     ((rating-r_ui)*i_accumulator_f[factor_index]*sig_derivative-self.user_reg*p_vf)
                            ...
                            for neighbor_index in range(item_k):
                                ...
                                if curr_neighbor != -1:
                                    ...
                                    # compute the gradient for neighbor v (curr_neighbor)
                                    local_gradient = data_item_similarity[i, neighbor_index]*\
                                                     ((rating-r_ui)*u_accumulator_f[factor_index]*sig_derivative-self.item_reg*q_kf)
    ...
    cdef void _add_user_sample_in_minibatch(self, long user):
        if not self.mini_batch_sampled_users_flag[user]:
            self.mini_batch_sampled_users_flag[user] = True
            self.mini_batch_sampled_users[self.mini_batch_sampled_users_counter] = user
            self.mini_batch_sampled_users_counter += 1


    cdef void _add_item_sample_in_minibatch(self, long item):
        if not self.mini_batch_sampled_items_flag[item]:
            self.mini_batch_sampled_items_flag[item] = True
            self.mini_batch_sampled_items[self.mini_batch_sampled_items_counter] = item
            self.mini_batch_sampled_items_counter += 1
    ...
    cdef void _apply_minibatch_updates_to_latent_factors(self):
        ...
        for num_sample_in_batch in range(self.mini_batch_sampled_items_counter):
            ...
            # update latent factors for the ITEMS
            for factor_index in range(self.n_factors):
                if self.factors_dropout_mask[factor_index]:
                    local_gradient_item = self.ITEM_factors_minibatch_accumulator[sampled_item, factor_index] / self.batch_size

                    #print('value {} in position {},{}'.format(self.ITEM_factors_minibatch_accumulator[sampled_item, factor_index],sampled_item,factor_index))
                    #input()
        ...
        for num_sample_in_batch in range(self.mini_batch_sampled_users_counter):
            ...
            # update latent factor for the USERS
            for factor_index in range(self.n_factors):
    ...
    cdef MSE_sample sampleMSE_Cython(self):
        ...
        if sample_positive:
            ...
            index = rand() % len(self.URM_train_coo_col)

            sample.user = self.URM_train_coo_row[index]
            sample.item = self.URM_train_coo_col[index]
            sample.rating = self.URM_train_coo_data[index]
    ...
    cdef BPR_sample sampleBPR_Cython_Pop_Correction(self, double[:] item_pop):

        """samples pos items in bpr adopting a popularity correction"""

        cdef BPR_sample sample = BPR_sample(-1,-1,-1)
        cdef long index, start_pos_seen_items, end_pos_seen_items, idx
        cdef double items_prob_sum, cumulative_items_prob_sum
        ...
        items_prob_sum = 0.0
        for idx in range(start_pos_seen_items, end_pos_seen_items):
            items_prob_sum += item_pop[self.URM_train_indices[idx]]
        items_prob_sum = ceil(items_prob_sum)

        stop_value = rand() % items_prob_sum

        cumulative_items_prob_sum = 0.0
        for idx in range(start_pos_seen_items, end_pos_seen_items):
            cumulative_items_prob_sum += item_pop[self.URM_train_indices[idx]]
            if cumulative_items_prob_sum < stop_value:
                continue
            else:
                break

        sample.pos_item = self.URM_train_indices[idx]
    ...
    cdef BPR_sample sampleBPR_Cython(self):
        ...
        sample.user = self.URM_train_coo_row[index]
        sample.pos_item = self.URM_train_coo_col[index]
    ...
    ##########################################################
    ##########  from here below there is the old version
    ##########  of bpr iteration, very fast but leading
    ##########  to slightly worse results, experimentally
    ##########################################################

    def epochIteration_Cython_old(self):
        ...
        cdef double H_i, H_j, W_u, cumulative_loss
        cumulative_loss = 0.0
        ...
        for numCurrentBatch in range(totalNumberOfBatch):
            ...
            # Uniform user sampling with replacement
            sample = self.sampleBPR_Cython_old()
            u = sample.user
            i = sample.pos_item
            j = sample.neg_item
            ...
            if((numCurrentBatch%5000000==0 and not numCurrentBatch==0) or numCurrentBatch==totalNumberOfBatch-1):
                print("Processed {} ( {:.2f}% ) in {:.2f} seconds. Sample per second: {:.0f}. BPR: {:.5f}".format(
    ...
    cdef BPR_sample sampleBPR_Cython_old(self):
