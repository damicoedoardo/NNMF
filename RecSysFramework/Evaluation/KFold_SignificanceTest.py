...
def compute_k_fold_significance(list_1, alpha_input, other_list,  this_label = None, other_label = None, verbose = True, log_file = None):
    ...
    if verbose:
        write_log("{}: {:.4f} ± {:.4f}".format(this_label, np.mean(list_1), np.std(list_1)), log_file=log_file)
    #
    ...
    if verbose:
        write_log("{}: {:.4f} ± {:.4f}".format(other_label, np.mean(other_list), np.std(other_list)), log_file=log_file)
    ...
    def run_significance_test(self, other_result_repository, metric = None, alpha = 0.05, verbose = True, dataframe_path = None):
        ...
        for metric in metric_list:
            ...
            if result_df is None:
                df_columns = ["metric", "difference_is_significant_pass", "difference_is_significant_superior"]
        ...
        if dataframe_path is not None:
            result_df.to_csv(dataframe_path, index=False)
