...
def compute_k_fold_significance(list_1, alpha, *other_lists):
    ...
    if len(other_lists) > 1:
        ...
        print("Applying Bonferroni correction for {} lists, original alpha is {}, corrected alpha is {}"
              .format(len(other_lists), original_alpha, alpha))
    ...
    for other_list_index in range(len(other_lists)):
        ...
        assert isinstance(other_list, list) or isinstance(other_list, np.ndarray), \
            "The provided lists must be either Python lists or numpy.ndarray"
        assert len(list_1) == len(other_list), \
            "The provided lists have different length, list 1: {}, list 2: {}".format(len(list_1), len(other_list))
        ...
        print("List {} t_statistic: {:.4f}, p_value: {:.4f}, alpha: {:.4f}. {}"
...
class KFoldResultRepository(object):
    ...
    def get_results_in_fold(self, fold):
        if fold < 0 or fold >= len(self._result_list):
            raise Exception("KFoldResultRepository: fold index out of bounds")
        return self._result_list[fold].copy()
    ...
    def run_significance_test(self, other_result_repository, metric=None, alpha=0.005):
        ...
        assert isinstance(other_result_repository, KFoldResultRepository), \
            "KFoldResultRepository: run_significance_test must receive another repository as parameter"
        assert other_result_repository.get_fold_number()== self.get_fold_number(), \
            "KFoldResultRepository: run_significance_test other repository must have the same number of folds"
